...
class Course(models.Model):
    ...
    cover_image = models.ImageField(
        ...
        null=True,
        help_text=_(
            "Каталог и страница курса. Если пусто — берётся «Картинка урока»: у junior — со "
            "второго урока; у pro — с третьего (финальный урок с «Слово автора» не трогаем)."
        ),
    )
...
class Lesson(models.Model):
    ...
    image = models.ImageField(
        ...
        null=True,
        help_text=_(
            "Junior: второй урок — запасная обложка курса. Pro: третий урок — обложка; шестой — "
            "отдельно «Слово автора» (<!-- AUTHOR_PHOTO -->). Иначе иллюстрация сверху, если урок "
            "не источник обложки."
        ),
    )
